...
Early approaches were lexicon-based. They worked by assigning positive or negative scores
to individual words and then combining those scores to get an overall sentiment. VADER~\cite{vader}
is a well-known example of this type of system. It was designed specifically for social media
text and handles punctuation, capitalization, and emoticons as signals of sentiment intensity. It
is fast and works reasonably well for clear, unambiguous text. However, it completely fails on
sarcasm and does not have any mechanism to say when it is unsure.
...
The deep learning era brought significant improvements. TextCNN~\cite{textcnn} applied convolutional
...
Several BERT-based systems have been published on GoEmotions~\cite{bert_goemotions, modernbert_goemotions, hybrid_emotion}. Most report accuracy in the
range of 0.46 to 0.58 on the full 28-class problem~\cite{goemotions_comparison, llm_goemotions}. On the simplified 3-class version (Positive,
Negative, Neutral), results are higher but still reflect the difficulty of the dataset~\cite{goemotions_google}. None of
these systems include any mechanism for detecting when a sample is too ambiguous to classify
reliably, although recent studies have explored fine-grained emotion detection using advanced
transformer models~\cite{bert_emotion, goemotions_comparison}.
...
Bayesian deep learning~\cite{bayesian} approaches model uncertainty by treating model weights as
...
Selective prediction~\cite{selective} allows a model to abstain from making a prediction when it is below
...
MARCH~\cite{march_ambiguity} have evaluated the intersection of ambiguity interpretation and
...
main tool is persistent homology~\cite{tda}, which tracks the birth and death of topological features as
...
TDA has been applied to NLP in several ways. Zhu~\cite{zhu} applied persistent homology to text
classification and showed that topological features captured information that was not present in
bag-of-words representations. Michel et al.~\cite{michel} analyzed the topology of sentence embedding
...
used as a tool for EEG processing~\cite{tda_eeg_tool}. Tools such as giotto-tda~\cite{giotto_tda},
...
The system uses SentenceTransformers~\cite{sbert} to convert text into dense vector representations.
...
Looking at all of the above, the main gaps in existing work are as follows.

No existing emotion detection system trained on GoEmotions includes an ambiguity detection
mechanism. They all always return a confident label.

Systems that do model uncertainty in NLP rely on the model's own output, not on the structure
of the data. If the model is confidently wrong, they do not help.

No published work uses local persistent homology at inference time as an ambiguity signal for
social media text.

This project fills all three gaps. It builds an emotion classifier, adds a topological ambiguity
detector based on local geometry, and combines the two into a system that knows when to
abstain from giving a confident answer.
