...
Lexicon-based methods like VADER assign scores to individual words and then combine those scores to get an overall sentiment. This is a well-known approach for early systems. These were designed specifically for textual data and handled punctuation, capitalization, and emoticons as signals of sentiment intensity.
Such systems are fast and work reasonably well for clear, unambiguous text. However, they
often fail on sarcasm and do not have any mechanism to say when they are unsure.
...
benchmark datasets but still struggled with complex language, negations, and short text segments where
context is limited.
...
released by Google and contains around 58,000 textual samples labelled by crowd workers
...
The third problem is the presence of \textbf{complex language}. Formal textual samples can contain nuanced
vocabulary, abbreviations, and expressions that are not well-represented in standard training data. Even in 
formal contexts, words and phrases carry emotional meaning that is often handled poorly
by models trained on generic text. The embedding models used to represent these words
...
Sentence-BERT Fine-tuned (2021)\tablefootnote{Sentence-BERT was introduced in 2019; the GoEmotions application results cited here are from subsequent fine-tuning evaluations.} & Transformer Embedding & Accuracy & $\sim$82\% \\ \hline
...
A key limitation shared by confidence-based selective prediction and Monte Carlo dropout is that both rely on the model's internal state to estimate uncertainty. If the model has learned to be systematically overconfident on a particular type of ambiguous input — as is common with majority-class-biased training — neither method will detect the uncertainty.
Topological signals, by contrast, are computed from the actual geometric distribution of training data rather than from model outputs. This makes them robust to the specific failure mode of confident-but-wrong predictions, which is particularly prevalent in imbalanced datasets like GoEmotions. Recent research on topological quantification of ambiguity~\cite{tda_ambiguity, blowfish} suggests
...
This approach has not been used for ambiguity detection in formal text, to the best
...
This limitation is significant because real-world formal text often contains subtle, implicit, or ambiguous emotions that cannot be accurately represented by a single label.
