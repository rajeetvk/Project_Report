...
\begin{itemize}
    \item \texttt{sentence-transformers}: For computing semantic embeddings using
    \texttt{all-mpnet-base-v2}.
    \item \texttt{scikit-learn}: For TF-IDF vectorization, nearest-neighbour indexing, and
    evaluation metrics.
    \item \texttt{xgboost}: For the classification models.
    \item \texttt{giotto-tda}: For Vietoris-Rips persistence computation and Persistence
    Landscapes.
...
The GoEmotions dataset (simplified version) is used. It was loaded from the HuggingFace
\texttt{datasets} library and cached locally so that training can proceed without an internet
connection after the first download.
...
The 28 emotion categories are mapped to three classes as shown in Table~\ref{tab:label_map}.
...
As shown in Table~\ref{tab:label_map}, the original GoEmotions labels are mapped to three
...
Table~\ref{tab:text_proc} shows the comparison between traditional NLP preprocessing and the
...
As shown in Table~\ref{tab:text_proc}, the design choice to avoid traditional preprocessing
...
Table~\ref{tab:ex_outputs} shows how the system responds to different types of inputs and which
rule was triggered.
...
As shown in Table~\ref{tab:ex_outputs}, the system's decision logic correctly differentiates
...
The first is a \textbf{point cloud visualization}. It shows the 24 nearest neighbours of the
...
The second is a \textbf{persistence diagram}. This shows all H0 and H1 features plotted as
...
The third is a \textbf{persistence landscape plot}. This is the smoothed, vectorized summary of
