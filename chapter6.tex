...
Table~\ref{tab:classperf} shows the classification results on the test set. These are the actual
...
As shown in Table~\ref{tab:classperf}, the system achieved an accuracy of 0.59 and a weighted
...
Table~\ref{tab:ambigresults} shows the behavior of the ambiguity detection on a set of test
...
As demonstrated in Table~\ref{tab:ambigresults}, the results match expectations well. Clear
...
Black box testing treats the system as a closed unit. The tester provides an input and checks
whether the output matches expectations, without looking at how the internal code works.
The goal is to verify that the system fulfills its requirements as observed from the outside.

In black box testing, test cases are designed based on the requirements and expected behaviors
defined in Chapter~3. The following categories of test cases were designed and executed.
...
As shown in Table~\ref{tab:blackbox}, all test cases passed. It is noted that ``i am fine''
...
The project was developed over a period of approximately four months. Table~\ref{tab:timeline}
...
detection in existing models. Reviewed TDA literature and persistent homology. \\
\addlinespace
Month 2 (February) &
Set up development environment. Implemented initial version with only embeddings and
XGBoost. Explored the GoEmotions dataset structure. Designed the label mapping. \\
\addlinespace
Month 3 (March) &
Added TF-IDF features. Integrated \texttt{giotto-tda} for topology computation. Debugged
shape errors in the topology pipeline. Experimented with neighbour counts and topology
parameters. Implemented the ambiguity detection rules. \\
\addlinespace
Month 4 (April) &
...
As detailed in Table~\ref{tab:timeline}, the project progressed steadily from the initial
literature review phase through to full pipeline implementation, threshold tuning, and formal
evaluation over the span of four months.
...
Table~\ref{tab:thresh} shows how the ambiguity rate changes as the loop thresholds are varied
...
As indicated in Table~\ref{tab:thresh}, the current thresholds of 14 and 10 were chosen because
