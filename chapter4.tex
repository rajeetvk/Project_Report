...
\begin{tikzpicture}[
  font=\small,
  box/.style={draw, rectangle, rounded corners=3pt, thick, align=center, minimum width=3.5cm, minimum height=1.0cm},
  widebox/.style={draw, rectangle, rounded corners=3pt, thick, align=center, minimum width=5.5cm, minimum height=1.0cm},
  arr/.style={-{Stealth[length=6pt]}, thick}
]

  % Nodes - Increased vertical and horizontal spacing to extend arrows
  \node[box] (input) at (0,0) {\textbf{Input Text}};
  ...
  \node[box] (xgb) at (0,-12.5) {\textbf{XGBoost Classifiers}\\[2pt]\textbf{(One-vs-Rest)}};
  
  \node[box] (ambig) at (0,-14.5) {\textbf{Ambiguity Detection}\\[2pt]\textbf{(Loop Count + Confidence Gap)}};
  
  \node[box] (output) at (0,-16.5) {\textbf{Final Output}\\[2pt]\textbf{(POS / NEG / NEU / AMBIG)}};
...
As shown in Figure~\ref{fig:arch}, the system processes the input text through three distinct
...
The input text is encoded using the \texttt{all-mpnet-base-v2} model from SentenceTransformers.
This produces a 768-dimensional dense vector that captures semantic meaning rather than just
word occurrences.

This representation allows semantically similar sentences to be close in the vector space even
if they use different words.

\subsection{TF-IDF Features}

TF-IDF (Term Frequency -- Inverse Document Frequency) captures the importance of individual
words and phrases. It complements embeddings by focusing on surface-level textual features.

A vocabulary of 3000 features (including unigrams and bigrams) is used. Stopwords are removed
to focus on meaningful words.

\subsection{Topological Features}

Topological features are extracted using Topological Data Analysis (TDA). For each input:
\begin{itemize}
    \item The 24 nearest neighbours are retrieved from the embedding space
    \item These points form a local point cloud
    \item Vietoris-Rips persistent homology is computed (H0 and H1)
\end{itemize}

H0 captures connected components, while H1 captures loops (holes). A Persistence Landscape is
derived to convert topology into a numerical feature vector.

Additionally, the number of H1 loops is used directly for ambiguity detection.
...
\begin{tikzpicture}[
  font=\small,
  box/.style={draw, rectangle, thick, align=center, minimum width=3.5cm, minimum height=1.5cm},
  widebox/.style={draw, rectangle, rounded corners=5pt, thick, align=center, minimum width=10cm, minimum height=0.8cm, fill=gray!10},
...
Figure~\ref{fig:feat} provides a detailed view of the feature extraction pipeline. The raw text
...
\begin{tikzpicture}[
  font=\small,
  decision/.style={draw, diamond, thick, align=center, aspect=1.5, minimum width=2.5cm},
  block/.style={draw, rectangle, thick, align=center, minimum width=3cm, minimum height=0.8cm},
  start/.style={draw, rectangle, thick, align=center, rounded corners=10pt, minimum width=3cm, minimum height=0.6cm},
  arr/.style={-{Stealth[length=5pt]}, thick}
]
  ...
  \node[block] (r4) at (0,-12.5) {Return Highest\\Probability Class};
...
As depicted in Figure~\ref{fig:ambig}, the ambiguity detection process follows a strict
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rectangle, rounded corners=5pt, thick, align=center,
              minimum width=3.2cm, minimum height=1.6cm, fill=gray!5},
  circ/.style={draw, circle, thick, align=center, minimum size=3.8cm},
  arr/.style={-{Stealth[length=6pt]}, thick}
]
...
As shown in Figure~\ref{fig:dfd0}, the system acts as a black box that accepts raw textual data
...
\begin{tikzpicture}[
  ...
  proc/.style={draw, circle, very thick, align=center, minimum size=3.0cm},
...
\node[box]          (out)  at (0,-4.5)  {\textbf{Final Output}\\\textbf{Label}};
...
Figure~\ref{fig:dfd1} details the internal flow of data. The process begins with
...
\begin{tikzpicture}[
  ...
  proc/.style={
    ...
    minimum size=3.0cm, text width=2.2cm
  },
  arr/.style={-{Stealth[length=6pt, width=4pt]}, thick}
]
...
% Result box
\node[box] (res) at (14, 0) {\textbf{Result}};
...
The Level 2 DFD (Figure~\ref{fig:dfd2}) illustrates the internal decision logic of the ambiguity
