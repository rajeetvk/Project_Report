...
\begin{itemize}
    \item \texttt{sentence-transformers}~\cite{sbert}: For computing semantic embeddings using
    \texttt{all-mpnet-base-v2}.
    \item \texttt{scikit-learn}: For TF-IDF vectorization, nearest-neighbour indexing, and
    evaluation metrics.
    \item \texttt{xgboost}: For the classification models~\cite{xgboost_eeg}.
    \item \texttt{giotto-tda}~\cite{giotto_tda}: For Vietoris-Rips persistence computation and Persistence
    Landscapes~\cite{giotto_docs}.
...
The GoEmotions dataset (simplified version) is used~\cite{goemotions, goemotions_google}. It was loaded from the HuggingFace
...
been subject to experimental analysis in recent literature~\cite{mdpi_annotation}.
