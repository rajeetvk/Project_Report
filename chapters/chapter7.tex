...
This project built an emotion detection system for formal text that does something most
...
\texttt{all-mpnet-\allowbreak base-v2} model capture the meaning of the text. TF-IDF features capture the
...
\begin{itemize}
    \item \textbf{Contextual Ambiguity Blind Spots:} The system is designed to detect semantic
    ambiguity based on geometric clustering. However, it cannot detect ``contextual'' ambiguity
    where the text is semantically clear but the underlying nuance or tone is hidden. For example, the
    phrase ``I am fine'' is contextually complex and often used to mask negative emotions, but since
    ...
    \item \textbf{Complex Terminology and Formal Nuance:} The system's understanding of emotion is tied to
    the semantic patterns captured by the base embedding model. Specialized vocabulary,
    highly technical abbreviations, or domain-specific jargon
    may not be correctly represented in the high-dimensional space if they were underrepresented
    during the model's pre-training. This limits the system's effectiveness on highly specialized
    formal textual environments.
...
\begin{itemize}
    ...
    creating a labelled ambiguity dataset. If a set of textual samples were annotated by
    ...
    \item \textbf{Nuance and Irony Detection:} Adding a dedicated nuance and irony
    detection module would address many of the cases where contextual ambiguity is not
    captured by the topological signal. Nuance in particular is common in formal textual data
    ...
    embeddings would make the system applicable to a much wider range of textual
    content.
