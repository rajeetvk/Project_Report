...
having about three times more samples than Positive and five times more than Negative. The textual samples come from formal sources, which contain nuanced and inherently ambiguous language. Even
...
GoEmotions corpus, the phrase ``I am fine'' and similar phrasings appear predominantly in
...
\begin{table}[H]
\centering
\caption{Effect of Threshold Configurations on Ambiguity Rate}
\label{tab:thresh}
\resizebox{\textwidth}{!}{
\begin{tabular}{ccccl}
\toprule
\textbf{Loop Extreme} & \textbf{Loop Moderate} & \textbf{Gap Threshold} & \textbf{Ambiguity Rate} & \textbf{Notes} \\
\midrule
10 & 7  & 0.30 & $\sim$38\% & Too aggressive \\
12 & 9  & 0.30 & $\sim$28\% & Slightly aggressive \\
14 & 10 & 0.30 & $\sim$21\% & Chosen \\
16 & 12 & 0.30 & $\sim$15\% & Misses real ambiguity \\
18 & 13 & 0.30 & $\sim$12\% & Too conservative \\
\bottomrule
\end{tabular}
}
\end{table}
...
The known limitations of the current system from an experimental and technical perspective are:

\begin{itemize}
    \item \textbf{Threshold Calibration:} The thresholds for ambiguity detection (14, 10, 0.30,
    and 0.10) were established through manual observation and sensitivity analysis rather than
    systematic optimization. In the absence of a large-scale, human-annotated dataset for
    ambiguity, these thresholds remain experimental and may require further tuning for different
    domains.

    \item \textbf{Inference Performance:} The requirement to compute Vietoris-Rips persistence and
    nearest-neighbour retrieval for every input at inference time introduces a computational
    bottleneck. While acceptable for interactive use, this overhead makes the current system
    less suitable for high-throughput, real-time stream processing compared to standard
    neural network classifiers.

    \item \textbf{Dataset-Specific Geometry:} The topological features are inherently tied to the
    geometric distribution of the GoEmotions training set. If the system is used on text from a
    radically different domain (such as technical manuals or medical reports), the semantic
    neighbourhoods may not be dense enough to produce meaningful topological signals.
\end{itemize}
