...
\begin{itemize}
    \item \texttt{sentence-\allowbreak trans\-for\-mers}~\cite{sentence_bert}: For computing semantic embeddings using
    \texttt{all-mpnet-\allowbreak base-v2}.
...
\texttt{ambiguity.\allowbreak py} for interactive inference.
...
The dataset has 43,410 training samples and 5,427 test samples. Each sample is a textual
sample with one or more labels from 28 emotion categories.
...
multi-label. A sample labelled as both ``joy'' and ``admiration'' would be mapped to Positive.
